...
from .models import Chat, Reputation, User
from .schemas import ReputationResponse, ReputationUpdate, UserScore
...
async def vote_user(data: ReputationUpdate, db: AsyncSession = Depends(get_db)):
    ...
    user_res = await db.execute(
        select(User).where(User.telegram_id == data.user.telegram_id)
    )
    ...
    chat_res = await db.execute(
        select(Chat).where(Chat.telegram_id == data.chat.telegram_id)
    )
    chat = chat_res.scalar_one_or_none()

    if not chat:
        chat = Chat(telegram_id=data.chat.telegram_id, title=data.chat.title)
        db.add(chat)
    else:
    ...
    if not reputation:
        ...
        if reputation.updated_at:
            ...
            if delta.total_seconds() < 60:
                raise HTTPException(
                    status_code=429, detail="Cooldown: please wait before voting again."
                )
...
async def get_top_users(
    chat_id: int, limit: int = 10, db: AsyncSession = Depends(get_db)
):
...
async def get_user_reputation(
    chat_id: int, user_id: int, db: AsyncSession = Depends(get_db)
):
